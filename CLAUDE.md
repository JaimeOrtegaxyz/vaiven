...
- `playground/index.html` — the studio (single self-contained file): live
  ...
  `vaiven.presets.json` shape (the per-project shelf). This is the UI surface —
  hold it to the design system below.
...
- Playground: `python3 -m http.server 4633` from repo root →
  `http://localhost:4633/playground/`. (No install; it's static.)
